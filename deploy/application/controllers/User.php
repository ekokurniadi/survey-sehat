...
class User extends MY_Controller
{
	...
	public function fetch_data()
	{
		...
		foreach ($fetch->result() as $rows) {
			...
			if ($rows->status == 0) {
				$button2 = "<a href=" . base_url('user/nonaktif/' . $rows->id) . " class='btn btn-icon btn-sm btn-flat icon-left btn-warning' onclick='javasciprt: return confirm(\"Are You Sure ?\")''></i>Nonaktif</a>";
			} else {
				$button2 = "<a href=" . base_url('user/aktifkan/' . $rows->id) . " class='btn btn-icon btn-sm btn-flat icon-left btn-warning' onclick='javasciprt: return confirm(\"Are You Sure ?\")''></i>Aktif</a>";
			}
			$button3 = "<a href=" . base_url('user/delete/' . $rows->id) . " class='btn btn-icon btn-sm btn-flat icon-left btn-danger' onclick='javasciprt: return confirm(\"Are You Sure ?\")''><i class='fa fa-trash'></i>Hapus</a>";
			$sub_array = array();
			$sub_array[] = $index;
			$sub_array[] = "<img src=" . base_url() . 'image/' . $rows->foto_profil . " class='img-fluid'>";
			$sub_array[] = $rows->status == 0 ? "Aktif" : "Nonaktif";
			$sub_array[] = $rows->nama;
			$sub_array[] = $rows->email;
			$sub_array[] = sha1($rows->password);
			$sub_array[] = $rows->no_telp;
			$sub_array[] = $rows->jenis_kelamin == 0 ? "Laki-Laki" : "Perempuan";
			$sub_array[] = formatTanggal($rows->tanggal_lahir);
			$sub_array[] = $rows->no_ktp;
			$sub_array[] = $rows->status_pernikahan;
			$sub_array[] = $this->db->get_where('master_pekerjaan', array('id' => $rows->pekerjaan))->row()->pekerjaan;
			$sub_array[] = $rows->tingkat_pendidikan;
			$sub_array[] = $this->db->get_where('master_provinsi', array('id_provinsi' => $rows->provinsi))->row()->provinsi;
			$sub_array[] = $this->db->get_where('master_kabupaten_kota', array('id' => $rows->kota))->row()->kabupaten_kota;
			$sub_array[] = $rows->tipe_tempat_tinggal;
			$sub_array[] = $this->db->get_where('operator_selular', array('id' => $rows->kartu_provider))->row()->nama_operator;
