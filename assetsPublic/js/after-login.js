...
(function($) {
    $(function() {
        ...
        backtop();
        boxlistFunc();
        if ($("div").is(".pix_diapo")) { diapoFunc() }
        tab2Func()
        faqFunc();
        checkFunc();
        GnaviFunc();
        $("#topics dt:even").addClass("bg_b");
        $(".point-table01 tr:even").css("background", "#e6e6e6");
        $(".point-table01 tr:odd").css("background", "#f6f6f6");
        $(".point-table02 tr:even").css("background", "#e6e6e6");
        $(".point-table02 tr:odd").css("background", "#f6f6f6");
    });
})(jQuery);
...
var boxlistFunc = function() {
    var $items = $('.header__inner .box-list > li');
    $items.bind("click", function() {
        if ($(this).hasClass("selected")) {
            var current = $('.header__inner .box-list > li.selected');
            $(".box-show", current).slideUp();
            ...
            $(".box-show", this).slideDown()
        }
    });
};
var tab2Func = function() {
    $('.tab-list02>li:nth-child(1)').addClass('selected');
    var $items = $('.tab-list02>li');
    $items.click(function() {
        $items.removeClass('selected');
        $(this).addClass('selected');
        var index = $items.index($(this));
        $('.tab-show').hide().eq(index).show();
    }).eq(0).click();
};
var tab3Func = function(key) {
    $('.tab-list03>li:nth-child(1)').addClass('selected');
    var $items = $('.tab-list03>li');
    $items.click(function() {
        $items.removeClass('selected');
        $(this).addClass('selected');
        var index = $items.index($(this));
        $('.show-content').hide().eq(index).show();
    }).eq(key).click();
};
...
function doPost(form_name, obj = null) {
    var form = "document." + form_name;
    objBtnSubmit = eval(form).getElementsByClassName('btn-submit');
    if (objBtnSubmit.length > 0) { objBtnSubmit[0].innerHTML = '<a href="javascript:void(0)" class="blue-bg"><img src="/application/templates/default/afterlogin/images/images/loading.gif" alt="" srcset=""></a>'; }
    eval(form).submit();
}

function login_KeyPress(e, form_name) { if (e.keyCode == 13) { doPost(form_name); return false; } }

function submitidea(userid) {
    if (userid.length != 0) {
        var idea_content = $.trim($('#comment_ques_textarea').val());
        var idea_answers = $.trim($('#comment_ans_textarea').val());
        var response = '';
        if (idea_content.length == 0) { alert("Tolong beri kami pendapat"); } else if (idea_answers.length == 0) { alert("Tolong beri kami jawaban"); } else if (idea_answers.search('\n') == -1) { alert("Mohon berikan setidaknya dua jawaban, setiap jawaban adalah garis."); } else {
            $.ajax({
                type: "POST",
                url: "/public/index/submitidea/",
                data: "userid=" + userid + "&idea_content=" + idea_content + "&idea_answers=" + idea_answers,
                success: function(responseText) {
                    if (parseInt(responseText) == 2) {
                        $(".box-survey .content-quick-survey").fadeOut();
                        $(".box-survey .thank-quick-survey").fadeIn();
                        rs_placeholderFunc();
                    } else { window.location = '/public/index/invalid-login/'; }
                }
            });
        }
    } else { alert("Login di butuhkan!"); }
}

function return_form_idea(userid) {
    $(".box-survey .content-quick-survey").fadeIn();
    $(".box-survey .thank-quick-survey").fadeOut();
}

function loadOccupation2() {
    $(document).ready(function() {
        $('.loading_img').show();
        setTimeout($("select[name='occupation']").load("/public/user/occupation", { CareerID: $("select[name='career']").val() }, $('.loading_img').fadeOut(2000)), 5000);
    });
}

function loadLivingCity() {
    var provinceId = $("select[name='province']").attr('value');
    var file = "/public/livingcity/livingcity_" + provinceId + ".txt";
    var rawFile = new XMLHttpRequest();
    rawFile.open("GET", file, false);
    rawFile.onreadystatechange = function() {
        if (rawFile.readyState === 4 && (rawFile.status === 200 || rawFile.status === 0)) {
            var allText = rawFile.responseText;
            $("select[name='livingcity']").html(allText);
        }
    }
    rawFile.send(null);
}

function loadChildOption() {
    $("#childBirthday_1_error").hide();
    $("#childBirthday_2_error").hide();
    $("#childBirthday_3_error").hide();
    $("#childBirthday_4_error").hide();
    $("#childBirthday_5_error").hide();
    numberChild = $("select[name='isChild']").val();
    if (numberChild != "" && numberChild != 6) {
        $("#childOption").show();
        $.ajax({ url: '/public/user/childoption', type: 'post', dataType: "html", data: { numberChild: numberChild }, success: function(data) { $("#childOption").html(data); } });
    } else {
        $("#childOption").html("");
        $("#childOption").hide();
    }
...
function get_like_social() {
    var fbGraphURL = 'https://graph.facebook.com/?ids=';
    var twApiURL = 'https://urls.api.twitter.com/1/urls/count.json?url=http://nusaresearch.net/public/index/single-quick-survey/qid/';
    var url = $("input[name='url_hidden']").val();
    var twObj = $(".count-twitter");
    $.ajax({
        url: fbGraphURL + url,
        method: 'get',
        dataType: 'json',
        success: function(data) {
            $.each(data, function(index, value) {
                if (value != 'undefined') {
                    tmp = value.id.replace('https://nusaresearch.net/public/index/single-quick-survey/qid/', '');
                    $('.fb_qid_' + tmp).html(value.shares);
                }
            });
        }
    });
    $.each(twObj, function() {
        ...
        $.ajax({ url: twApiURL + qid + '&callback=?', method: 'get', dataType: 'json', success: function(data) { obj.html(data['count']); } });
    });
}
var placeholderFunc = function(key) {
    var as_placeholder = 'Silahkan masukkan pilihan jawaban dengan urutan abjad minimum 2 pilihan jawaban.\nTuliskan dalam baris untuk setiap pilihan jawaban.\nContoh: Bagaimana pendapat Anda tentang Nusaresearch\na. Survei menarik\nb. Informasi berguna\nc. Desain bagus\nd. Tidak ada hal khusus';
    $('#comment_ans_textarea').attr('value', as_placeholder);
    $('#comment_ans_textarea').focus(function() { if ($(this).val() === as_placeholder) { $(this).attr('value', ''); } });
    $('#comment_ans_textarea').blur(function() { if ($(this).val() === '') { $(this).attr('value', as_placeholder); } });
};
var rs_placeholderFunc = function(key) {
    var as_placeholder = 'Silahkan masukkan pilihan jawaban dengan urutan abjad minimum 2 pilihan jawaban.\nTuliskan dalam baris untuk setiap pilihan jawaban.\nContoh: Bagaimana pendapat Anda tentang Nusaresearch\na. Survei menarik\nb. Informasi berguna\nc. Desain bagus\nd. Tidak ada hal khusus';
    $('#comment_ans_textarea').attr('value', as_placeholder);
    $('#comment_ques_textarea').attr('value', '');
};;
